...
// を保証する（loadOldestSpillPath()参照。以前は全件比較で厳密な最古を探していたが、
// ヒープ断片化の原因になっていたため撤去した。2026-09-01、
...
// 捨てて新しいデータを残す。上記の理由により「退避ファイル群のいずれか」は
// 厳密な最古ではなくなったが、捨てる対象は既にRAMキューより古い(=spillへ退避済み)
// データに限られるため、「最新のRAMキューデータを優先して残す」という本来の狙いは
// 変わらない。既定は false でこの例外は発動しない
