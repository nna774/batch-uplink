...
// 送信順序: LittleFSの退避ファイルを先に、次にRAMキューの古い順。退避ファイル側は
// 厳密な最古優先ではなく「新しいデータの流入で古いバックログが飢餓しない」ことだけ
// を保証する（loadOldestSpillPath()参照。データにタイムスタンプが埋め込まれている
// ため、配送順の厳密さ自体は下流の正しさに影響しない）。
//
// 例外1: dropOldestWhenFull=true の時だけ、LittleFSも一杯でこれ以上退避できない
// 場面に限り、既存データ（退避ファイル群のいずれか、無ければRAMキューの先頭）を
// 捨てて新しいデータを残す。退避ファイル群からの選択は厳密な最古ではないが、
// 捨てる対象は既にRAMキューより古い(=spillへ退避済み)データに限られるため、
// 「最新のRAMキューデータを優先して残す」という狙いは保たれる。既定は false で
// この例外は発動しない（呼び出し側が明示的に選ばない限り不変条件は変わらない）。
//
...
class Uploader {
 public:
  ...
  // キュー中のバッチの開始時刻[us]（送信順序と同じ基準：退避ファイルがあれば
  // そのいずれか、無ければRAMキューの先頭。退避ファイル側は厳密な最古ではない
  // ——上のファイル冒頭コメント参照）。キューが空なら false。表示で
  // 「どれだけ遅れているか」の目安を出す用途を想定（呼び出し側が現在時刻との
  // 差を取る）。
